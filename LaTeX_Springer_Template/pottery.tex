...
For each vertex in the mesh, we use a $m \times n$ matrix $M$ to store radius values, where number of row $m = s_{v} + 1$ and the number of column $n = s_{r}$ respectively. The base radius $r_{base_{i}}$ of each vertex $v_{ij}$ in row $i$ can be written as: 
\begin{equation}
\begin{split}
a^2+b^2=c^2 \\
a^2+b^2=c^2
\end{split}
\end{equation}
where  is a factor controls the shape blending between a  cylinder (when ) and a semi-ellipsoid (when ) (Fig 2).
...
To add noise to the centre, we use random  and  to present the centre . Then we sum the values for each radius , where  is the radius for each circle, and  is individual radius for each vertex.(Fig 3) We can get the radius value  in the matrix  for each vertex, and calculate the vertex position based on the radius values in the matrix:
\begin{equation}
\begin{split}
a^2+b^2=c^2 \\
a^2+b^2=c^2
...
\end{split}
\end{equation}
where  is the thickness value of the clay mesh. In mesh generation phase, the initial value of  is 1; in mesh deformation phase, the value of  can be adjusted by user.
...
\end{split}
\end{equation}
where  is a damping factor controlling the radial smoothing effect.
...
\end{split}
\end{equation}
where  and  are previous height and thickness values before every deformation respectively;  is a damping factor for height.
...
\end{split}
\end{equation}
where  is the vertical distance between the handle and ;  and  are inner and outer handle radius respectively. 
...
\end{split}
\end{equation}
where  is the number of adjacent vertices of ;  is a factor controlling the radial smoothing effect.
